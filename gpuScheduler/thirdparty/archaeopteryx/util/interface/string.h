...
__device__ operator streamoff() const {
	return __off_;
}

__device__ _StateT state() const {
	return __st_;
}
__device__ void state(_StateT __st) {
	__st_ = __st;
}
...
__device__ fpos operator+ (streamoff __off) const {
	fpos __t(*this); __t += __off; return __t;
}
__device__ fpos& operator-=(streamoff __off) {
	__off_ -= __off; return *this;
}
...
__device__ basic_string& operator=(const_pointer __s) {
	return assign(__s);
}
...
__device__ void resize(size_type __n) {
...
__device__ const_pointer c_str() const {
	return data();
}
