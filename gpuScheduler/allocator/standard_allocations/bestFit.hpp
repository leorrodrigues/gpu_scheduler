...
namespace Allocator {
bool BestFit(Builder* builder,  Container* container, std::map<unsigned int, unsigned int> &allocated_task,consumed_resource_t* consumed){

	std::priority_queue<Host*> (builder->getHosts().begin(), builder->getHosts().end());
	//
