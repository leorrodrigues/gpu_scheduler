...
typedef struct {
	...
	std::map<std::string, float> mFloat; ///< map to represent all the float and double variables.
	...
	int mIntSize, mFloatSize, mStringSize, mBoolSize;///< variables to represent theis respective map size, used to reduce the function overloads call (removing the call of .size() in the map).
} Resource;
