...
template <typename T> void AHP::buildNormalizedmatrix(T *v) {
	...
	iterateFunc(&AHP::buildNormalizedmatrix<Hierarchy<VariablesType, WeightType>::Criteria>,v);
}
...
template <typename T> void AHP::deleteMatrix(T* v){
	...
	matrix=NULL;
	v->setMatrix(NULL);
	iterateFunc(&AHP::deleteMatrix<Hierarchy<VariablesType,WeightType>::Criteria>,v);
}
...
template <typename T> void AHP::deleteNormalizedMatrix(T* v){
	...
	iterateFunc(&AHP::deleteNormalizedMatrix<Hierarchy<VariablesType,WeightType>::Criteria>,v);
}
...
template <typename T> void AHP::checkConsistency(T *v) {
	...
	if (IR[size] > 0) {
		...
		// RC = (Lambda_max - n) / (~Lambda_max - n), the ~Lambda_max can be calculated through
		// ~Lambda_max = 2.7699*n-4.3513, thus
		// RC = (Lambda_max - n) / (2.7699 * n - 4.3513 - n ), simplifying
		// RC = (Lambda_max - n) / (1.7699 * n - 4.3513)
...
void hierarchyParser(auto *dataObjective,auto ahp) {
	for (auto &hierarchyObject : dataObjective->value.GetObject()) {
		if (strcmp(hierarchyObject.name.GetString(), "name") == 0) {
			ahp->hierarchy->addFocus(strToLower(
	...
	for (auto &arrayAlternative : dataAlternative->value.GetArray()) {
		...
		for (auto &alt : arrayAlternative.GetObject()) {
			...
			if (alt.value.IsNumber()) {
				...
				alternative->setResource(name, strToLower(std::string(alt.value.GetString())));
			}
		}
	}
}
...
void AHP::acquisition() {
	...
	for_each(
		...
		[&sheetsNames,this](Hierarchy<VariablesType, WeightType>::Criteria *c) mutable {
		// Get the name off all sheet nodes that aren't boolean
		if (this->hierarchy->getResource()->mBool.count(c->getName()) == 0) {
		...
		for(auto it=alt.begin(); it!=alt.end(); it++) {
			...
			for(auto it2=alt.begin(); it2!=alt.end(); it2++) {
				if ((*it)->getResource()->mInt.count((*sIt)->getName()) > 0) {
					result = ((*it)->getResource()->mInt[(*sIt)->getName()] -  (*it2)->getResource()->mInt[(*sIt)->getName()])/resultValues[(*sIt)->getName()];
				} else if ((*it)->getResource()->mWeight.count((*sIt)->getName()) > 0) {
					result = ((*it)->getResource()->mWeight[(*sIt)->getName()] -  (*it2)->getResource()->mWeight[(*sIt)->getName()])/resultValues[(*sIt)->getName()];
				} else {
					if((*it)->getResource()->mBool[(*sIt)->getName()] == (*it2)->getResource()->mBool[(*sIt)->getName()]) {
						result=1;
					}else{
						(*it)->getResource()->mBool[(*sIt)->getName()] ? result=9 : result= 1/9.0;
					}
				}
				if(result==0) {
					result=1;
...
//void AHP::run(std::vector<Hierarchy<VariablesType,WeightType>::Alternative*> alt){
