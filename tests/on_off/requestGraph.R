...
## 1070 TI ##
#######################################
gtxDataOnline <- read.table("data/gtx1070ti/request-none-lyon-50k-fcfs-online-lyon-50k.log",header=FALSE,sep=" ")
gtxDataOffline <- read.table("data/gtx1070ti/request-none-lyon-50k-fcfs-offline-lyon-50k.log",header=FALSE,sep=" ")
...
## Titan XP ##
#######################################
titanXPDataOnline <- read.table("data/titanXP/request-none-lyon-50k-fcfs-online-lyon-50k.log",header=FALSE,sep=" ")
titanXPDataOffline <- read.table("data/titanXP/request-none-lyon-50k-fcfs-offline-lyon-50k.log",header=FALSE,sep=" ")
...
## 2080 TI ##
#######################################
rtxDataOnline <- read.table("data/rtx2080ti/request-none-lyon-50k-fcfs-online-lyon-50k.log",header=FALSE,sep=" ")
rtxDataOffline <- read.table("data/rtx2080ti/request-none-lyon-50k-fcfs-offline-lyon-50k.log",header=FALSE,sep=" ")
...
#######################################
## Plot the walltime line graph ##
#######################################
walltimeLinePlot <- ggplot(data=dt, aes( x=submission, y=walltime, color= type, linetype= gpu))+
    # data=dt, aes( x=submission, y=walltime, color= interaction(type, gpu)))+
    geom_line() +
    theme_classic()+
    theme(
...
#######################################
## Plot the delay line graph ##
#######################################
delayLinePlot <- ggplot(data=dt, aes( x=submission, y=delay, color= type))+
    # data=dt, aes( x=submission, y=walltime, color= interaction(type, gpu)))+
    geom_line() +
    theme_classic()+
    theme(
...
#######################################
walltimeByDelayPlot <- ggplot(data=dt, aes( y= delay, x= walltime, color= type, linetype = gpu))+
    geom_point() +
    theme_classic()+
    theme(
        legend.position="right",
...
tiff("walltime_line_50k.tiff",width= 3500, height= 1200, units="px", res=400,compression = 'lzw')
plot(walltimeLinePlot)
dev.off()

tiff("walltime_cdf_50k.tiff",width= 3500, height= 1200, units="px", res=400,compression = 'lzw')
plot(walltimeCDFPlot)
dev.off()

tiff("delay_line_50k.tiff",width= 3500, height= 1200, units="px", res=400,compression = 'lzw')
plot(delayLinePlot)
dev.off()

tiff("delay_cdf_50k.tiff",width= 3500, height= 1200, units="px", res=400,compression = 'lzw')
plot(delayCDFPlot)
dev.off()

tiff("walltime_delay_50k.tiff",width= 3500, height= 1200, units="px", res=400,compression = 'lzw')
