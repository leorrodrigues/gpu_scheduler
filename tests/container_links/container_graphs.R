...
metrics <- c("algorithm","submission","id","delay","u-container","u-link","runtime","bw")
metrics_labels <- c("","","","Delay (events)","Container Utility", "Link Utility","","")


for(name in names){
    for(metrics_index in 4:6){
        data1 <- read.table(sprintf("requests/pod%s-bw1.json",name), sep=" ")
        data1$bw <- "<=01Mbps"
        data2 <- read.table(sprintf("requests/pod%s-bw25.json",name), sep= " ")
        data2$bw <- "<=25Mbps"
        data3 <- read.table(sprintf("requests/pod%s-bw50.json",name), sep= " ")
        data3$bw <- "<=50Mbps"
