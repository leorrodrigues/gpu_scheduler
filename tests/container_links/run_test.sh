    ...
    for p in 0 50 100; do
        for bw in 0 25 50; do
            for m in ahpg topsis; do
                ...
                ./gpuscheduler.out --test 4 -s 44 -m $m -c $g --request-size $p --bw $bw
            done;
            for sm in best_fit worst_fit first_fit; do
                echo "Fat Tree k= 48 Algoritmo $sm Cluster $g Request-size $rs"
                ./gpuscheduler.out --test 4 -s 44 --standard-allocation $sm -c $g --request-size $p --bw $bw
            done;
        done;
    done;
done;
