		...
		for p in 0 50 100; do
				...
				for m in ahpg topsis; do
					echo "Fat Tree k=$ft Algoritmo $m Cluster $g Request-size $rs"
		        	       ./gpuscheduler.out --test 4 -s $ft -m $m -c $g --request-size $p --bw $bw --data-type $dt --debug error
		      		done;
	        	   	for sm in best_fit worst_fit; do # first_fit; do
                			echo "Fat Tree k=$ft Algoritmo $sm Cluster $g Request-size $rs"
		               		./gpuscheduler.out --test 4 -s $ft --standard-allocation $sm -c $g --request-size $p --bw $bw --data-type $dt --debug error
		       	 	done;
		       done;
		done;
	done;
