...
#include <boost/math/distributions/fwd.hpp> //  add skew_normal distribution to fwd.hpp!
...
{
/*
   No closed expression for mode, so use max of pdf.
 */
...
/*
 *  No closed expression for mode, so use f'(x) = 0
 */
template <class RealType, class Policy>
...
inline RealType mode(const skew_normal_distribution<RealType, Policy>& dist)
{
	...
	// : make the search bounds smarter, depending on the shape parameter
